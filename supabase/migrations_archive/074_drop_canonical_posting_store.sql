-- Remove the abandoned Scout/Boards posting store. Discover now reads the
-- GitHub internship feeds directly and only persists user feed interactions.

drop table if exists public.posting_source_observations cascade;
drop table if exists public.postings cascade;
