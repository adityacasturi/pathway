-- Roll back Scout hybrid RAG: search index, sync queue, hybrid RPC, description_text.

drop function if exists public.hybrid_search_postings(
  text,
  extensions.vector,
  integer,
  text,
...
drop table if exists public.posting_search_sync_queue;
drop table if exists public.posting_search_documents;
