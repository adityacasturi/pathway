-- Multiple recruiting seasons per scraped posting (e.g. Winter + Spring).

alter table public.scraped_postings
  add column if not exists seasons text[] not null default array['Summer']::text[];

update public.scraped_postings
set seasons = array[season]::text[]
where seasons is null
   or cardinality(seasons) = 0;

alter table public.scraped_postings
  alter column seasons set default array['Summer']::text[];

alter table public.scraped_postings
  drop constraint if exists scraped_postings_seasons_valid;

alter table public.scraped_postings
  add constraint scraped_postings_seasons_valid
  check (
    cardinality(seasons) >= 1
    and seasons <@ array['Summer', 'Fall', 'Spring', 'Winter']::text[]
  );

comment on column public.scraped_postings.seasons is
  'One or more recruiting seasons; `season` mirrors the primary entry for legacy queries.';
...
as $$
  ...
  where status not in ('open', 'closed')
     ...
     or not (seasons <@ array['Summer', 'Fall', 'Spring', 'Winter']::text[])
